System.register(["angular2/core", "../services/partidas.service", "angular2/router"], function(exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
        var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
        if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
        else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
        return c > 3 && r && Object.defineProperty(target, key, r), r;
    };
    var __metadata = (this && this.__metadata) || function (k, v) {
        if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
    };
    var core_1, partidas_service_1, router_1;
    var NuevoJugadorComponent;
    return {
        setters:[
            function (core_1_1) {
                core_1 = core_1_1;
            },
            function (partidas_service_1_1) {
                partidas_service_1 = partidas_service_1_1;
            },
            function (router_1_1) {
                router_1 = router_1_1;
            }],
        execute: function() {
            NuevoJugadorComponent = (function () {
                function NuevoJugadorComponent(_partidasService) {
                    this._partidasService = _partidasService;
                }
                NuevoJugadorComponent.prototype.onSubmit = function () {
                };
                NuevoJugadorComponent = __decorate([
                    core_1.Component({
                        selector: "nuevoJugador",
                        templateUrl: "app/views/nuevoJugador.html",
                        styleUrls: ['assets/css/registro.css'],
                        providers: [partidas_service_1.PartidasService],
                        directives: [router_1.ROUTER_DIRECTIVES]
                    }), 
                    __metadata('design:paramtypes', [partidas_service_1.PartidasService])
                ], NuevoJugadorComponent);
                return NuevoJugadorComponent;
            }());
            exports_1("NuevoJugadorComponent", NuevoJugadorComponent);
        }
    }
});
//# sourceMappingURL=nuevoJugador.components.js.map